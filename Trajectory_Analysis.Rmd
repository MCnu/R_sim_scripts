...
mmsdplot(c("FLE_URA3", "FLE_u90_b90"))
```
...
output_cluster_comp <- data.frame(ID = "SimZip", exp = mean_id_frame[, "Pair_ID"], dCorr = magcor_frame[, "mag_PCC"], Delta_Theta = mean_clust_theta_frame[, "mean_theta"], avg_Dist = mean_id_frame[, "interdist"])
...
max_plot <- ggplot(data = big_pos_frame) +
  stat_density_2d_filled(aes(x = x_pos, y = y_pos, fill = ..level.., color = ..level..), contour_var = "ndensity") +
